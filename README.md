...
تم تطوير التطبيق ليعمل بسلاسة على بيئات تشغيل متعددة:

Mobile: دعم كامل لأندرويد 14 و15.

Windows: نسخة مخصصة لسطح المكتب (Desktop App) لأداء أسرع على الحاسوب.

Web: إمكانية التشغيل عبر المتصفح (Web App) للوصول السريع من أي مكان.

📊 اللغات المستخدمة (Languages)
TypeScript: 98.0% (المنطق المالي والذكاء الاصطناعي للمحفظة)

HTML: 1.3% (هيكلة واجهات الويب والويندوز)

Java: 0.7% (تكاملات نظام الأندرويد)
...
🤖 مميزات الذكاء الاصطناعي (AI Features)
تم دمج محركات الذكاء الاصطناعي (مثل Google Gemini API) لتقديم تجربة فريدة:

التصنيف الذكي (AI Auto-Categorization): تحليل نصوص المصروفات وتصنيفها تلقائياً (طعام، فواتير، استثمار) باستخدام الـ NLP.

التحليل التنبؤي (Predictive Spending): يتوقع التطبيق مصروفاتك القادمة بناءً على بياناتك التاريخية باستخدام نماذج التنبؤ: $$ \hat{y} = \beta_0 + \beta_1 x_1 + \epsilon $$

المستشار المالي الذكي (AI Financial Advisor): شات بوت مخصص داخل التطبيق يجيب على استفساراتك المالية ويقترح عليك خططاً للتوفير بناءً على دخلك.

اكتشاف الشذوذ المالي (Anomaly Detection): تنبيهك فوراً في حالة وجود مصروفات غير معتادة أو خارجة عن نمط
